...
namespace AutoAnswer.Services
{
    public class AtomService : IAtomService
    {
            ...
            var tasks = Atoms.Select(atom =>
            {
                return Task.Run(() =>
                {
                    try
                    {
                        ...
                        var packet = this.ParternAddination.ToArray();
                        packet[8] = itemId[0];
                        packet[9] = itemId[1];
                        packet[10] = itemId[2];
        ...
        public bool IsBuyAtom(byte[] packet)
        {
            ...
            if (packet.Length != 0x5F) return false;
            try
            {
                var pkgs = packet.ToList();
                var pkg = pkgs.Take(4).ToArray().vnToStringHex() + " " + pkgs.Skip(7).Take(PacketContants.BUY_ATOM.Length - 4).ToArray().vnToStringHex();
