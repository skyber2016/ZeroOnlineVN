    ...
    public partial class Query : BaseQuery<Query>
    {
        ...
        private Dictionary<string, object> BuildDictionaryFromObject(object data, bool considerKeys = false)
        {
            ...
            foreach (var property in props)
            {
                ...
                var colAttr = property.GetCustomAttributes(typeof(ColumnAttribute), true).FirstOrDefault(x => x != null) as ColumnAttribute;
                var colKey = property.GetCustomAttributes(typeof(KeyAttribute), true).Any(x => x != null);

                var name = colAttr?.Name ?? property.Name;

                dictionary.Add(name, value);

                if (considerKeys && colAttr != null && colKey)
                {
