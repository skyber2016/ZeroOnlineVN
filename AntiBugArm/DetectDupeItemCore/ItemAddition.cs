...
internal static class ItemAddition
{
        ...
        for (int i = await GetLastLineNumber(e); i < lines.Length; i++)
        {
            ...
            try
            {
                try
                {
                    ...
                    Item item2 = item;
                    item2.IsValid = await ItemValid(item, user, e, i + 1);
                    if (item.IsValid)
                    {
                        continue;
                    }
                    _logger.Error((object)$"user_id={user.UserId} username={user.Username} robot_id={item.RobotId} account_id={user.AccountId} item_id={item.SecondId} file_name={e.Name} line_number={i + 1}");
                    if (user == null)
                    {
                        continue;
                    }
                    ...
                    {
                        string notification = GetNotification(item, user, e, i + 1);
    ...
    private static async Task<bool> ItemValid(Item item, UserInfo user, FileSystemEventArgs e, int lineNumber)
    {
        ...
        {
            _logger.Info((object)$"Detected user_id={user.UserId} username={user.Username} robot_id={item.RobotId} account_id={user.AccountId} item_id={item.SecondId} file_name={e.Name} line_number={lineNumber} used item not existed");
            return false;
        }
