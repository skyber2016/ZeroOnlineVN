...
namespace CORE_API.Middlewares
{
    public class CryptoMiddleware
    {
        ...
        private readonly PrometheusReporter prometheus;
        public CryptoMiddleware(RequestDelegate next, IOptions<CryptoSettings> options, ILoggerManager logger, IWebHostEnvironment env, PrometheusReporter _prometheus)
        {
            this._next = next;
            this.CryptoSettings = options.Value;
            this.Logger = logger;
            this.Env = env;
            this.prometheus = _prometheus;
        }
        /// <summary>
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            if (this.IsSwagger(httpContext) || !httpContext.Request.Path.StartsWithSegments("/api") || path == "/eib-api/actuator/prometheus")
            {
            ...
            catch (Exception ex)
            {
                ...
                sw.Stop();
                this.prometheus.RegisterRequest();
                this.prometheus.RegisterResponseTime(path, method, httpContext.Response.StatusCode);
            }
            finally
            {
