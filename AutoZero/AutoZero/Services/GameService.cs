...
namespace AutoZero.Entities
{
    public class GameService : IDisposable
    {
        ...
        public void GameToMid_Connected(object sender, TcpClient e)
        {
            ...
            {
                this.Game = e;
                //this.UnitOfWork.ViewModel.Accounts.Add(new AccountModel
                //{
                ...
                try
                {
                    ...
                    this.MidClient.Connect(this.AppSettings.IpServer, this.AppSettings.PortGameServer);
        ...
        public void ServerToMid_Receiver(object sender, Message e)
        {
            try
            {
                ...
                //});
                //task.Start();
                var taskGetUsername = new Task(() =>
                {
                    if (e.Data.Includes(PacketContants.USERNAME))
                    {
                        this.Username = UserHelper.GetUsername(e.Data.ToArray());
                        this.Account = new AccountModel { Username = this.Username, FuncName = "testttttt" };
                        this.UnitOfWork.ViewModel.Dispatcher.Invoke(() =>
                        {
                            this.UnitOfWork.ViewModel.Accounts.Add(Account);
                        });
                    }
                });
                taskGetUsername.Start();
        ...
        public void Dispose()
        {
            try
            {
                ...
                this.UnitOfWork.ViewModel.Accounts.Remove(Account);
                GC.Collect();
            }
            catch (Exception ex)
            {
