        ...
        public void GameToMid_Connected(object sender, TcpClient e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    this.MidClient.Connect(this.AppSettings.IpServer, this.AppSettings.PortLoginServer);
                }
                catch (Exception ex)
                {
        ...
        public void ServerToMid_Receiver(object sender, Message e)
        {
            try
            {
                ...
                if (e.Data.GetPacketType(2).vnEquals(PacketContants.LoginResponse))
                {
                    ...
                    var portLoginServer = string.Join(" ", BitConverter.GetBytes(this.UnitOfWork.AppSettings.PortGameServer));
                    var portLoginMid = string.Join(" ", BitConverter.GetBytes(this.UnitOfWork.AppSettings.PortGameMid));
                    var ipServer = this.UnitOfWork.AppSettings.IpServer;
                    var ipMid = this.UnitOfWork.AppSettings.IpMid;
